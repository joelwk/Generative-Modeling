...
def process_paired_data(data, comment_input='text',comment_target='text'):
    ...
    # Ensure special tokens are in the list of texts for tokenizer fitting
    ...
    for comment_seq, response_seq in zip(comment_seqs, response_seqs):
        # Prepend [start], append [sep] between comment and response, and [end] after response
        input_seq = [start_token_id] + comment_seq + [sep_token_id] + response_seq + [end_token_id]
        # Target sequence for the response, starting from the first token of the response
        target_seq = response_seq + [end_token_id]
        input_seqs.append(input_seq)
        target_seqs.append(target_seq)

    # Padding sequences after adding special tokens
